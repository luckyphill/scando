...
for code in stockPriceData:
	...
	x = [float(stockPriceData[code][n][4]) for n in range(1,len(stockPriceData[code]))]
	up_days = [round(x[n]-x[n-1],4) if x[n]-x[n-1] >= 0 else 0 for n in range(1,len(x))]
	down_days = [round(x[n-1]-x[n],4) if x[n-1]-x[n] >= 0 else 0 for n in range(1,len(x))]
	...
	rsi_time_data = zip([stockPriceData[code][n][0] for n in range(period +1,len(stockPriceData[code]))],rsiData[code])
